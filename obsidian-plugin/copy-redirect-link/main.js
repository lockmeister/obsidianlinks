...
module.exports = class CopyRedirectLinkPlugin extends Plugin {
  ...
  async renderNote(file, includedPaths) {
    const source = (await this.app.vault.read(file)).replace(/^\uFEFF?---\r?\n[\s\S]*?\r?\n(?:---|\.\.\.)\r?\n?/, '');
    const markdown = await this.inlineImageEmbeds(source, file.path);
    ...
    container.querySelectorAll('script,iframe,object,embed,form,.metadata-container,.frontmatter').forEach((element) => element.remove());
    [...container.querySelectorAll('.callout.is-collapsible')].reverse().forEach((callout) => {
      const details = document.createElement('details');
      details.className = `callout callout-${callout.dataset.callout || 'note'}`;
      if (!callout.classList.contains('is-collapsed')) details.open = true;
      const summary = document.createElement('summary');
      summary.textContent = callout.querySelector('.callout-title-inner')?.textContent?.trim() || 'More';
    ...
    if (renderedTitle?.textContent?.trim().toLowerCase() === file.basename.trim().toLowerCase()) renderedTitle.remove();
  ...
  async buildHtml(files, title) {
    ...
    for (const file of files) {
      ...
      sections.push(`<article class="note" id="note-${id}"><h1>${escapeHtml(file.basename)}</h1>${await this.renderNote(file, included)}</article>`);
    }
